...
import './web-experiment-variant.js';

/**
 * @typedef {import('./web-experiment-variant.js').WebExperimentVariant} WebExperimentVariantElement
 */
 ...
 *
 * @fires web-experiment:variant-assigned - Dispatched *after* a variant has been definitively chosen and displayed.
 * The event bubbles and is composed.
 * Event detail contains: `{ experimentId: string, variantId: string, trigger: 'pre-selected' | 'load' | 'select' }`
 * - 'pre-selected': A variant was shown because it had the `selected` attribute.
 * - 'load': A variant was shown because it was found in client-side storage.
 * - 'select': A variant was randomly selected and stored client-side.
 *
